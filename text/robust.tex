...
\jna{How can we "see" that the number of outliers is small? How can we identify "the knob region" in the table?}
To confirm that the means are statistically representing the same distribution the t-tests were also run. This is summarized in \refTable{tab:statTest} below. It is easy to see that the number of outliers is small, except for the knob region. 
...
\jna{What are these 18 "settings"? What changes from one setting to the next?}
...
\jna{The axis in Figure~\ref{fig:fdllrep} need to be named. Otherwise no one can read the figure.}
