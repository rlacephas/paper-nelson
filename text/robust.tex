...
From three independent experiments, the first one running $10$-times, after that running $100$-times, and then, the last one running $1000$-times, there is enough information to discard the outliers. They can be discarded as this experiment confirms, because there is no difference on their means, and also the behavior of the program remained unchanged. To make sure that these are robust measures, some simple statistics were run, finding the mean, the median, the standard-deviation from the mean (std-mean), and the standard-deviation from the median (std-median). The simple statistical results are shown in \refTable{tab:robustTest}. Also t-tests were run on each sample pairs to verify if their means were the same, the results are shown in \refTable{tab:ttest}.
