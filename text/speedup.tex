...
For the \gcc\ benchmark the results show a speedup of $2.52 \%$ over \llvm\, and a $3.17 \%$ speedup over Never, whereas \llvm\ achieved a $0.67 \%$ speedup over Never, for the whole input set. This result is summarized in \refTable{tab:speedupgcc} below. The results are normalized by the baseline Never (no inlining).
...
The slowdown over \llvm\ measured is of $2.43 \%$ for \gcc, as shown in \refTable{tab:slowdowngcc}. \refFigure{fig:slowdowngcc} shows the normalized running time for the slowdown measured for \gcc.
...
\begin{figure}
  \centering
  \includegraphics[width=1.00\linewidth]{Figures/slowdowngccall}
